...
export function MultiChannelFlowChart({
  ...
  // Bind to real incoming data or graceful fallback for initial view
  const p1 = data?.[0] || { period: "2024", totalLabel: "26 signals" };
  const p2 = data?.[1] || { period: "2025", totalLabel: "36 signals" };
  const p3 = data?.[2] || data?.[0] || { period: "2026", totalLabel: "34 signals" };

  const p1Label = p1.totalLabel || (p1.total ? `${p1.total} signals` : "26 signals");
  const p2Label = p2.totalLabel || (p2.total ? `${p2.total} signals` : "36 signals");
  const p3Label = p3.totalLabel || (p3.total ? `${p3.total} signals` : "34 signals");
  ...
  return (
    <div className="bg-white rounded-[28px] p-6 sm:p-7 border border-slate-100/90 shadow-[0_4px_25px_-5px_rgba(0,0,0,0.03)] flex flex-col justify-between space-y-5 transition-all duration-300 hover:shadow-[0_12px_35px_-5px_rgba(0,0,0,0.06)] hover:-translate-y-0.5">
      ...
      <div className="w-full relative pt-1 pb-1">
        <svg
          className="w-full h-auto max-h-[215px] overflow-visible select-none"
          viewBox="0 0 500 215"
          fill="none"
        ...
        >
          <defs>
            ...
            {/* Ambient drop shadow filter for the peak active pillar */}
          ...
          {/* 1. RIBBON STREAM FILLS (Smooth Flow Curves between exact Layer Heights) */}
          {/* Amber Stream */}
          <path
          ...
          {/* 2. CRISP WHITE SEPARATION BORDER LINES */}
          <path
          ...
          <g>
            ...
            <g className="transition-transform duration-200 hover:scale-102 origin-bottom cursor-pointer">
            ...
            <text x="78" y="190" textAnchor="middle" fill="#64748B" fontSize="11" fontWeight="600">
              {p1Label}
            </text>
          </g>
          ...
          <g>
            ...
            <text x="250" y="190" textAnchor="middle" fill="#64748B" fontSize="11" fontWeight="600">
              {p2Label}
            </text>
          </g>

          {/* --- PILLAR 3 (Active Peak) (X: 398 to 448) --- */}
          <g filter="url(#pillarShadow)">
            ...
            {/* Bottom Peak Black Pill Badge with Real Active Count */}
            <rect x="378" y="176" width="90" height="24" rx="12" fill="#09090B" />
            <text x="423" y="192" textAnchor="middle" fill="#FFFFFF" fontSize="11" fontWeight="800" letterSpacing="-0.02em">
              {p3Label}
            </text>
          </g>
        </svg>
